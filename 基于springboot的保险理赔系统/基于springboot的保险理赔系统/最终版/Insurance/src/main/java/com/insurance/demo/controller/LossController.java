...
public class LossController {
    ...
    public DataVo queryAllByLimit(int page, int limit) {
        ...
        try {
            List<Loss> losses = lossService.queryAllByLimit((page - 1) * limit, limit);
            dataVo.setData(losses);
            dataVo.setCount(lossDao.queryAll(null).size());
            dataVo.setCode(0);
        } catch (Exception e) {
            dataVo.setMsg("查询失败");
            dataVo.setCode(500);
