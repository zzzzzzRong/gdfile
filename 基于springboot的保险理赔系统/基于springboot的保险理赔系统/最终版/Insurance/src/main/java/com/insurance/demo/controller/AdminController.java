...
public class AdminController {
    ...
    /**
     * 功能1：查询type为待定损员审核的案件
     * 此时案件中没有定损员信息，案件表信息不全，可能有bug
     * @return
     */
    @GetMapping("queryType1")
    public DataVo queryType1(int page, int limit){
        return adminService.queryType1(page,limit);
    }
    /**
     * 功能2：查询type为待委员会审核的案件
     * 非委员会成员直接修改保险、标信息，可能有bug
     * @return
     */
    @GetMapping("queryType2")
    public DataVo queryType2(int page, int limit){
        return adminService.queryType2(page, limit);
    } /**
     * 功能23：查询type=10，已经结束的案件
     * 非委员会成员直接修改保险、标信息，可能有bug
     * @return
     */
    @GetMapping("queryType3")
    public DataVo queryType3(int page, int limit){
        return adminService.queryType3(page, limit);
    }
 /**
     * 功能23：查询type=10，已经结束的案件
     * 非委员会成员直接修改保险、标信息，可能有bug
     * @return
     */
    @GetMapping("queryType4")
    public DataVo queryType4(int page, int limit){
        return adminService.queryType4(page, limit);
    }
